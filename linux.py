...
USE_SCAPY = False  # Disabled by default (install manually if needed)
...
def ping_ip(ip):
    """Pings an IP using a Linux-compatible command."""
    try:
        ping_cmd = ["ping", "-c", "1", "-W", "1", ip] if OS_NAME != "windows" else ["ping", "-n", "1", "-w", "1000", ip]
...
def check_open_ports(ip):
    ...
    for port in PORTS_TO_CHECK:
        try:
            sock = socket.create_connection((ip, port), timeout=0.5)  # Faster timeout
            sock.close()
